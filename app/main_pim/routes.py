...
@main_pim_bp.route('/import_attributes_products/', methods=['POST'])
@jwt_required()
def import_attributes_products():

    if 'file' not in request.files:
        return {'status': 'error', 'message': 'No file part'}, 200

    file = request.files['file']

    if file.filename == '':
        return {'status': 'error', 'message': 'No se ha seleccionado un archivo'}, 200

    if file and not file.filename.endswith('.csv'):
        return {'status': 'error', 'message': 'El archivo seleccionado no es un CSV'}, 200
    ...
    db.connect()
    ahora = datetime.now().strftime('%d/%m/%Y - %H:%M:%S')
    current_app.logger.info(f'{ahora} - Importando atributos con productos')

    filepath = os.path.join(current_app.config.get('UPLOAD_FOLDER',''),file.filename)
    file.save(filepath)

    df = pd.read_csv(filepath,sep=";",encoding='UTF-8',dtype=str, index_col=False)
    if df.shape[1] < 3:
        return {'status': 'error', 'message': 'Número de columnas incorrectas'}, 200
    PandasGen.backup_products_categories(db)
    pag = PandasGen(db)
    try:
        msg = pag.import_overwrite_attributes_products(df)
    except Exception as e:
        current_app.logger.error(f'{ahora} - Error en la importación de atributos con productos {str(e)}')
        return {'status': 'error', 'message': f'Error en la importación de atributos con productos, err: {str(e)}', 'err': str(e)}
