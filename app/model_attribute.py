...
class Attribute():
    ...
    def deselect_category(self,category:str,productid:str):
        if not self._db.exists('tbdatributos','fldidatributo',self._id):
            raise Exception(f'Attribute {self._id} no encontrado')
        if not self._db.exists('tbdarticulos0','fldidarticulo',productid):
            raise Exception(f'Articulo {productid} no encontrado')
    ...
    def select_category_product(self,category:str,productid:str,colors:list[tuple[str,str]]=[]):
        ...
        if not self._db.exists('tbdarticulos0', 'fldidarticulo', productid):
            raise Exception(f'Articulo {productid} no encontrado')
        if not colors and not self._selected_color:
            return {'status':'error','message':f'No se han encontrado colores para el producto: {productid}.'}
        if category not in self._categories:
            self.set_categories()
            if category not in self._categories:
                raise Exception(f'Categoria {category} no encontrada en el atributo con id: {self._id} Y nombre: {self._name}.')
        if not self._selected_color:
            for color in colors:
                ...
                if self._db.fetchone() and not self._ismultiselectable:
                    ...
                    q = "select * from tbdatributosproductoscategorias where fldidatributo = ? and fldidarticulo = ? and fldidcolor = ? and fldcategoria = ?"
                    self._db.execute(q, [self._id, productid, color[0], category])
